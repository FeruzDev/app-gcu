...
//    private ExpertRepository expertRepository;
    ...
    @PreAuthorize(value = "hasAnyRole('ROLE_ADMIN', 'ROLE_WORKER')")
    @GetMapping
    public HttpEntity<?> getAll(@RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "10") Integer size, @CurrentUser User user) {
        if (user.getRoles().size() == 1 && user.getRoles().get(0).getName().equals(RoleName.ROLE_WORKER)){
            return ResponseEntity.ok(new ApiResponse(true, "", jeweleryRepository.findAllByExpertIdOrderByCreatedAtDesc(user.getId(), PageRequest.of(page, size))));
        } else {
    ...
    public HttpEntity<?> saveOrUpdate(@RequestBody ReqJewelery reqJewelery, @CurrentUser User currentUser) {
        try {
            ...
            if (reqJewelery.getId() == null) {
                ...
                jewelery.setSerial(reqJewelery.getSerial());
            }
    ...
    public HttpEntity<?> checkSerial(@PathVariable String serial) {
        try {
            if (jeweleryRepository.findAllBySerial(serial).isEmpty()) {
                return ResponseEntity.ok(new ApiResponse(true, "success"));
            } else {
                return ResponseEntity.ok(new ApiResponse(false, "Jewelery with this serial exists"));
            }
